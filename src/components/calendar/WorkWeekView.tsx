...
export const WorkWeekView = ({ currentDate, plans, setModal, addOrUpdate, enumMaps, customerMap, deviceMap, defaultPlanDuration }) => {
    ...
    const plansByDay = useMemo(() => {
        ...
        weekDays.forEach((day, index) => {
            map.set(index, plans.filter(p => p.scheduledFrom && areDatesEqual(new Date(p.scheduledFrom), day)));
        });
        return map;
    }, [plans, weekDays]);
    ...
    const handleDrop = (e, dayIndex) => {
        ...
        // Clamp y to be within the bounds of the column
    ...
    const handleColumnClick = (e, dayIndex) => {
        ...
        const fromDate = new Date(weekDays[dayIndex]);
    ...
    return html`
        <div class="week-view-timeline">
            <div class="timeline-header">
                <div class="time-gutter-header"></div>
                ${weekDays.map(day => html`
                    <div class="timeline-day-header ${areDatesEqual(day, today) ? 'today' : ''}">
            ...
            <div class="timeline-body">
                ...
                <div class="timeline-main" style=${{ height: `${hours.length * HOUR_HEIGHT}px` }}>
                    ...
                    ${Array.from({length: 5}, (_, i) => i).map(dayIndex => {
                        const dayPlans = plansByDay.get(dayIndex) || [];
                        const timedPlans = dayPlans.filter(p => p.scheduledFrom.includes('T'));

                        return html`
                            <div 
                                class="day-column ${dragOverColumn === dayIndex ? 'drag-over' : ''}"
                                ondragover=${handleDragOver}
                                ondrop=${e => handleDrop(e, dayIndex)}
                                ondragenter=${() => setDragOverColumn(dayIndex)}
